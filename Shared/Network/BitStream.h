...
// Directly copied from RakNet's NativeTypes.h
...
// Directly copied from RakNet's RakNetTypes.h
...
// Directly copied from RakNet's RakNetDefines.h
...
#define _copysign copysign
	...
	public:
		...
		template <class templateType>
		bool ReadNoSwap(templateType &outTemplateVar);
		...
		{
			unsigned short l = (unsigned short)strlen(inStringVar);
			Write(l);
		...
		{
			size_t length = wcslen(inStringVar);
			WriteCasted<unsigned short>(length);
			WriteAlignedBytes((const unsigned char*)inStringVar, (const unsigned int)length * 2);
		}
...
#pragma warning(disable:4244)   // '=' : conversion from 'unsigned long' to 'unsigned short', possible loss of data
	...
	/// V:Multiplayer addition: For some reasons endian swap causes reading problems, and getting rid of it in the
	/// regular BitStream::Read causes crash, so for now add another reading function
	template <class templateType>
		inline bool BitStream::ReadNoSwap(templateType &outTemplateVar)
		{
#ifdef _MSC_VER
#pragma warning(disable:4127)   // conditional expression is constant
#endif
...
/*	template <>
	...
	{
		// Directly copied from RakString::Deserialize
		bool b;
		unsigned short l;
		b = Read(l);
		if (b && l>0)
			b = ReadAlignedBytes((unsigned char*)varString, l);
	...
	{
		// Directly copied from RakWString::Deserialize
		size_t length;
		ReadCasted<unsigned short>(length);
		if (length>0)
		{
			return ReadAlignedBytes((unsigned char*)varString, (const unsigned int)(length + 1)*2);
		}
		else
		{
			wcscpy(varString, L"");
